...
// ---------------------------------------------------------------------------
// virtual:anvil/tailwind-sources
// ---------------------------------------------------------------------------

/**
 * Generate Tailwind CSS @source directives for all tool packages.
 *
 * Auto-discovers tools from the scope tree and generates @source directives
 * so Tailwind scans their source files for utility class usage.
 *
 * Import this virtual CSS in your app's stylesheet:
 * ```css
 * @import 'virtual:anvil/tailwind-sources';
 * ```
 */
export function generateTailwindSourcesModule(config: AppConfig): string {
  const tools = collectTools(config)
  const lines: string[] = [
    '/* Auto-generated by @ydtb/anvil-toolkit — Tailwind source paths for all tools */',
  ]
